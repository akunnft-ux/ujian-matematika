...
│   └── seed.sql         ← data demo
└── docs/                ← PRD, arsitektur, setup Supabase
```
...
3. Admin buka tab **Ujian** di `admin.html` → klik **Aktifkan** → status jadi `aktif` dan
   **satu token** (`TKN-XXXX`) dibuat untuk ujian itu (dipakai semua siswa).
...
dan RPC function di `supabase/schema.sql` (backend).

## Format soal (.docx)

Soal bernomor (`1.`, `2.`, ...), opsi `A.`–`E.`, dan bagian **KUNCI JAWABAN** di akhir.
Baris `KODE SOAL`, `JENJANG` (SD/SMP/SMA/SMK), dan `MAPEL` di awal dokumen menjadi identitas satu paket:
```
